...
// Returns Map<filename, [{hash, date, author, message}]>
function buildGitIndex() {
  const index = new Map()
  let out
  try {
    out = execSync(
      `git log --format="COMMIT\x1f%H\x1f%aI\x1f%an\x1f%s" --name-only -- "glossary/*.md"`,
...
function buildHistory(terms) {
  ...
  for (const term of terms) {
    ...
    const commits = gitIndex.get(filename) ?? []
